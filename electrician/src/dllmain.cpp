...
bool main() {
	...
	for (;;) {
		const auto pixel = GetPixel(device_context, 760, 393);

		const shared::color::c_color<int> color{ (pixel >> 16) & 0xff, (pixel >> 8) & 0xff, pixel & 0xff };
		if (color != shared::color::c_color<int>{ 140, 130, 124 })
			continue;
